...
import { APIWorkerEnv, JobStages } from "types"
import { z } from "zod"
...
export class TrainModel extends OpenAPIRoute {
  ...
  async handle(c: Context<{ Bindings: APIWorkerEnv }>) {
    ...
    // Generate a random pack name by combining an animal with a number
    const packName =
    ...
    const processedFiles = await Promise.all(
      files.map(async (file, index) => {
        ...
        // put to r2 under a folder `train/random-name/01.png`
